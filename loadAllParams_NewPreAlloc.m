...
nTotEmbryos = nTotEmbryos + length(vitGroup{i});
end
nTotEmbryos

% Memory preallocation
% add NaNs equivalent to number of embryos in experiment
stdevList = NaN*ones(1,nTotEmbryos);
vitGroupOut = NaN*ones(1,nTotEmbryos);
cellClumpOut = NaN*ones(1,nTotEmbryos);
blastFormOut = NaN*ones(1,nTotEmbryos);
diffStart = NaN*ones(1,nTotEmbryos);
diffMid = NaN*ones(1,nTotEmbryos);
diffEnd = NaN*ones(1,nTotEmbryos);
diffdiff1 = NaN*ones(1,nTotEmbryos);
diffdiff2 = NaN*ones(1,nTotEmbryos);
darkPercent = NaN*ones(1,nTotEmbryos);
rProfileOut = NaN*ones(20,nTotEmbryos);
pnDistFromCenter1 = NaN*ones(1,nTotEmbryos);
pnDistFromCenter2 = NaN*ones(1,nTotEmbryos);
pnSeparation = NaN*ones(1,nTotEmbryos);
    ...
    if (exptsToLoad(i) && exist(fileNameAssoc{i}, 'file'))
        ...
        for j = 1:length(vitGroup{i})
            ...
            if isfield(eval(['Group' num2str(groupNum) ...
                ...
                vitGroupOut(nEmbryo) = vitGroup{i}(j);
                cellClumpOut(nEmbryo) = cellClump{i}(j);
                blastFormOut(nEmbryo) = blastForm{i}(j);
                ...
                stdevList(nEmbryo) = mean(stdevCurr);
                ...
                diffStart(nEmbryo) = dh7(round(length(dh7)*.25));
                diffMid(nEmbryo) = dh7(round(length(dh7)*.6));
                diffEnd(nEmbryo) = dh7(round(length(dh7)*.8));
                diffdiff1(nEmbryo) = dh7(round(length(dh7)/2)) - dh7(round(length(dh7)*.25));
                diffdiff2(nEmbryo) = dh7(round(length(dh7)*.8)) - dh7(round(length(dh7)/2));
                rProfileOut(:,nEmbryo) = h7';
                ...
                pnDistFromCenter1(nEmbryo) = pnDist1;
                pnDistFromCenter2(nEmbryo) = pnDist2;
                pnSeparation(nEmbryo) = pnSep;
        ...
        %Could not load data set, increment nEmbryo
        nEmbryo = nEmbryo + length(vitGroup{i});
    end
end
